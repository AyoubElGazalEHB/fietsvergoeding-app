...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const { trajectory_id, ride_date, direction, portion, declaration_confirmed } = req.body;
  ...
  direction,
  portion,
  employeeId  // Changed from req.user.land to employeeId
);
    const result = await pool.query(
      `INSERT INTO rides (employee_id, trajectory_id, ride_date, direction, portion, km_total, amount_euro, declaration_confirmed, declaration_date) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) 
       RETURNING *`,
      [employeeId, trajectory_id, ride_date, direction, portion, km, amount, declaration_confirmed || false, declaration_confirmed ? new Date() : null]
    );
...
router.get('/month/:yearMonth', authenticate, async (req, res) => {
  try {
    ...
    if (!/^\d{4}-\d{2}$/.test(yearMonth)) {
      return res.status(400).json({ message: 'Ongeldig formaat. Gebruik YYYY-MM' });
    }
